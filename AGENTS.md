...
- **Binary Placement Model**: `binary_placements` is a separate presentation hierarchy; it never changes `users.referred_by_id` or commission levels. Each non-root user occupies one Left or Right position, and a unique `(parent_user_id, side)` index limits every position to two children. Registration locks placement allocation and fills the earliest open slot breadth first (Left before Right) **within the direct sponsor's placement branch**, so every referred user remains visible beneath their sponsor. Referrals without a code start a new tree. Placement depth may differ from sponsorship depth when a sponsor has more than two direct referrals.
- **Binary Migration Gate**: New databases receive the table and sponsor-branch backfill through `drizzle/0002_faithful_metal_master.sql`. Databases that already have the table from schema push but lack placement rows use the reviewed one-time `scripts/backfill-binary-placements.sql`. The backfill validates that every sponsored user is reachable beneath their sponsor and never runs from a request. Check migration history before using `yarn db:migrate` on an existing database.
- **Legacy Data Repair**: Reviewed one-time scripts in `scripts/repair-referral-lineage.sql`, `scripts/backfill-wallets.sql`, and `scripts/backfill-binary-placements.sql` reconcile stale closure rows, create missing zero-balance wallets, and assign missing binary positions. They are not run automatically on requests or deployment.
...
- **API Route Endpoints (`src/app/api/auth/`)**:
  - `POST /api/auth/register`: Immediate account activation, wallet initialization, 10-level sponsorship attribution, and atomic Left/Right placement inside the direct sponsor's branch while email verification is disabled.
